...
static void		ft_is_prog_name_rel_path(t_msh_command *cmd)
{
	char	*path;
...
static void		ft_is_prog_name_abs_path(t_msh_command *cmd)
{
	cmd->prog_stats = malloc(sizeof(struct stat));
	cmd->stats_rcode = stat(cmd->prog_name, cmd->prog_stats);
	if (cmd->stats_rcode >= 0 && !ft_handle_err(cmd))
	{
		ft_fork_and_start(cmd->prog_name, g_msh_params->input->prog_prms, \
			g_msh_params->cur_environ, 1);
	}
	else if (cmd->stats_rcode < 0)
	{
...
void			ft_start_prog(void)
{
	...
	if (g_msh_params->input->prog_name[0] == '/')
		...
		ft_is_prog_name_rel_path(g_msh_params->input);
	else
		ft_start_prog_path();
}
